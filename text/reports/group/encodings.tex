...
To model energy consumption realistically, we introduce acceleration mechanics.
...
Depending how fast the train moves and what it does, it consumes different amounts of energy. In the encoding, energy is deivided into three distinct kinematic states:
\begin{itemize}
...
energy(ID,T2,E) :- action(train(ID),A,T),
                   ...
                   E = 100 * CF * 1/S * 1/S.
  \end{verbatim}
  \item \textbf{Turning:} When an agent navigates a curve, centrifugal forces demand additional work. This is modeled by applying a 10\% penalty to the base straight-line movement cost, resulting in the calculation $E = 100 \cdot CF \cdot \left(\frac{1}{S}\right)^2 \cdot \frac{11}{10}$.
  \begin{verbatim}


% To simulate regenerative braking capabilities,
% the "costs" of braking are actually gains in this case
