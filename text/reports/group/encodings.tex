...
The \texttt{\#minimize} directive from the baseline encoding was removed, as the it increased the computation time of the acceleration encoding.
...
\begin{itemize}
  \item \textbf{Moving Straight:} For an object moving in a straight line \cite[p.~169]{nolting_grundkurs_2006}, its kinetic energy is calculated using this formula:
  \begin{equation}
  \label{eq:kinetic_energy}
  T = \frac{m}{2} \dot{x}^2 
  \end{equation}
  
  whereby $\dot{x}$ is the velocity and $m$ the mass.
  
    This is reflected in the encoding with the formula $E = 100 \cdot CF \cdot \left(\frac{1}{S}\right)^2$, whereby $\frac{1}{S}$ is the speed (being the inverse of Flatland's inverse speed), and $CF$ the train's energy cost factor incorporating its mass.
  To prevent loss of precision, the multiplier $100$ is introduced:
...
energy(ID,T2,E) :- action(train(ID),A,T),
                   ...
                   E = 100 * CF * 1/S * 1/S.
  \end{verbatim}
  \item \textbf{Turning:} In physical railway operations, negotiating a curve generates substantial mechanical resistance at the wheel-rail interface \cite[p.~2--3]{wu_curving_2022}. Because Flatland operates on a discrete grid with only sharp 90° curves, traditional empirical calculations cannot be applied meaningfully. To model this energy loss within the solver's constraints, we abstract curving resistance using a heuristic value of 10\% multiplied to the previously explained formula for energy consumption on a straight track. This value was chosen because it penalizes unnecessary zigzagging heavily enough without making essential turns prohibitively expensive. This results in the calculation $E = 100 \cdot CF \cdot \left(\frac{1}{S}\right)^2 \cdot \frac{11}{10}$.
  \begin{verbatim}
